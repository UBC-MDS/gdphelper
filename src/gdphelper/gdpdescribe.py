...
def gdpdescribe(df, x, y, stats=["mean", "sd", "median"], dec=2):
    ...
    Examples
    --------
    >>>gdpdescribe(df, "GDP", "year", stats=["mean", "median", "sd", "min", "max", "range_", "q75", "q25", "iqr", "nas"], dec=3)
    """
    # Avoid cientific notation in the display, use decimal points
    ...
    for i in stats:
        if i in [
            ...
            raise ValueError(
                "The statistic to calculate should be ['mean','median', 'sd', 'min', 'max', 'range_', 'q75', 'q25,'iqr', 'nas'] "
            )
